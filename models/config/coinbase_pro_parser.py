...
from .default_parser import defaultConfigParse
from .default_parser import isCurrencyValid
from .default_parser import merge_config_and_args
...
def parser(app, coinbase_config, args=None):
    ...
    if "api_key" in coinbase_config or "api_secret" in coinbase_config:
        print(
            '* "api_key / secret" should be saved in a separate file referred as "api_key_file"'
        )
        print(
            "* the file should be a simple text file with key/secret/passwd on separate lines\n"
        )
